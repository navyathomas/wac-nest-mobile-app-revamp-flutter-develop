...
class PartnerPreferenceAgeWheelTo extends StatefulWidget {
  ...
  @override
  State<PartnerPreferenceAgeWheelTo> createState() =>
      _PartnerPreferenceAgeWheelToState();
}

class _PartnerPreferenceAgeWheelToState
    extends State<PartnerPreferenceAgeWheelTo> {
  FixedExtentScrollController? controller;
  @override
  void initState() {
    controller = FixedExtentScrollController(initialItem: 0);
    context.read<PartnerPreferenceProvider>().getAgeWheeToIndex(context);
    super.initState();
  }
      ...
      builder: (context, provider, child) {
        if (!provider.isAgeIndexUpdated) {
          CommonFunctions.afterInit(() {
            controller!.animateToItem(provider.ageToIndex,
                ...
                builder: (context, value, child) {
                  return ListWheelScrollView.useDelegate(
                    ...
                    onSelectedItemChanged: (int index) {
                      ...
                      provider.ageDataToValue = ageData?[index];
                      // context
                      //     .read<PartnerPreferenceProvider>()
                      //     .updateAgeTo(ageData?[index]);
                    },
                    useMagnifier: false,
                    itemExtent: 60.h,
                    magnification: 2.0,
  ...
  Widget build(BuildContext context) {
    return Selector<PartnerPreferenceProvider, AgeList?>(
      selector: (context, provider) => provider.ageListDataTo,
          ...
          child: Column(
            children: [
              ...
              ApplyCancelBtn(onApplyTap: () {
                ...
                context.read<PartnerPreferenceProvider>().updateAgeTo(
                    context.read<PartnerPreferenceProvider>().ageDataToValue);
                context
                    .read<PartnerPreferenceProvider>()
                    .getAgeWheeToIndex(context);
                context.rootPop;
              }, onCancelTap: () {
