...
class PartnerPreferenceHeightWheelTo extends StatefulWidget {
  final BuildContext context;
  final ValueNotifier<int>? selectedIndex;
  final String? title;
  const PartnerPreferenceHeightWheelTo(
      {Key? key, this.selectedIndex, required this.context, this.title})
      : super(key: key);

  @override
  State<PartnerPreferenceHeightWheelTo> createState() =>
      _PartnerPreferenceHeightWheelToState();
}

class _PartnerPreferenceHeightWheelToState
    extends State<PartnerPreferenceHeightWheelTo> {
  FixedExtentScrollController? controller;
  @override
  void initState() {
    controller = FixedExtentScrollController(initialItem: 0);
    context.read<PartnerPreferenceProvider>().getHeightWheeToIndex(context);
    super.initState();
  }
      ...
      builder: (context, provider, child) {
        if (!provider.isAgeIndexUpdated) {
          CommonFunctions.afterInit(() {
            controller!.animateToItem(provider.heightToIndex,
                ...
                builder: (context, value, child) {
                  return ListWheelScrollView.useDelegate(
                    ...
                    onSelectedItemChanged: (int index) {
                      ...
                      provider.heightDataToValue = heightData?[index];
                    },
                    useMagnifier: false,
                    itemExtent: 60.h,
                    magnification: 2.0,
    ...
    return Consumer<PartnerPreferenceProvider>(
      builder: (context, provider, childWidget) {
            ...
            children: [
              ...
              ApplyCancelBtn(
                onApplyTap: () {
                  ...
                  context.read<PartnerPreferenceProvider>().updateHeightTo(
                      context
                          .read<PartnerPreferenceProvider>()
                          .heightDataToValue);
                  context
                      .read<PartnerPreferenceProvider>()
                      .getHeightWheeToIndex(context);
                  context.rootPop;
                },
              )
            ],
        ...
        return CustomOptionBtn(
          title: widget.title ?? '',
          selectedValue: provider.heightDataTo?.height ?? '',
          onTap: () {
